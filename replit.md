...
| **Communications** | **Live** — 3 tabs (Announcements, Handover Notes, Notices); compose dialog; reply composer; pin/archive actions; summary bar; 8 seeded messages. Routes: /api/v1/communications/* (local:8080). |
| **Superadmin Console** | **Live** — Platform-wide management panel under Settings → Superadmin tab (visible to superadmin only). 5 sections: Overview (stats + audit log), Businesses (CRUD + module toggles), Users (CRUD + credential management), Permissions (Role Matrix 6×25 + Assignments filter/edit + User View), New Property Wizard (4-step: info → modules → admin account → complete). Routes: /api/v1/admin/* (local:8080). |
| Settings | Live (owner-only business settings; superadmin console above) |
| Session Debug | Live |
